...
class ClassicMetaController:
    def __init__(
        ...
        if self.static_params.num_players != 1:
            raise Exception("Only one isolated agent is supported. Use num_spoofed_experts to simulate zeroed experts")
        self.num_spoofed_experts = num_spoofed_experts
        ...
        fake_static_params = self.static_params.replace(num_players=self.num_spoofed_experts + 1)  # pyright: ignore
        self.aux = AuxLossNet(
            # np.prod(self.observation_space.spaces[0].shape)  # pyright: ignore
            np.prod(get_player_data_obs_shape(fake_static_params))  # pyright: ignore
        )
    ...
    def fix_obs(self, obs):
        """
        Takes in single observation, returns
        """
        static_params = self.static_params.replace(num_players=self.num_spoofed_experts + 1) # pyright: ignore
        player_obs = jnp.zeros((1, self.num_envs,) + get_player_data_obs_shape(static_params))
        return (obs[0], player_obs)
    ...
    def train_some_episodes(
        ...
        def rollout_step(carry, _step):
            ...
            def eval_agent(rng, agent_param, next_lstm_state):
                rng, _rng = jax.random.split(rng)
                ...
                # )
                action = jax.random.categorical(_rng, logits).reshape(1,-1)
                return (
            ...
            rng, _rng = jax.random.split(rng)
            value, action, logprob, next_lstm_states = eval_agent(
                _rng,
                agent_params,
                next_lstm_states,
            )
            ...
            next_obs = self.fix_obs(next_obs)
            ...
            def do_epoch(carry, epoch):
                ...
                def do_minibatch(carry, start):
                    ...
                    init_lstm_state = jax.tree.map(
                        lambda state: state[mbenvinds], init_lstm_states
                    )
    ...
    def train(self, model_params=None, model_opt_params=None):
...
if __name__ == "__main__":
    ...
    with open("/Users/ericye/code/craftax_tests/params_learner_half.p", "rb") as f:
        params = pickle.load(f)
    with open("/Users/ericye/code/craftax_tests/opt_learner_half.p", "rb") as f:
        opt = pickle.load(f)
    # keep first 3 agents
    metacontroller = ClassicMetaController(
        num_spoofed_experts=3,
        static_parameters=StaticEnvParams(num_players=1),
        num_envs=10,
        num_minibatches=1,
        num_steps=200,
        num_iterations=5,
    ...
    params, opt_states, log = metacontroller.train(params, opt)
